...
(* --- inlined from PrintAST (only name_of_external + its helper
   name_of_chunk are used by PrintClight; this removes the PrintAST
   dependency, whose builtin_arg printers don't match this extraction). --- *)
...
let name_of_external = function
  | EF_external (name, sg) ->
      Printf.sprintf "extern %S" name
  | EF_builtin (name, sg) ->
      Printf.sprintf "builtin %S" name
  | EF_runtime (name, sg) ->
      Printf.sprintf "runtime %S" name
  | EF_vload chunk ->
      Printf.sprintf "volatile load %s" (name_of_chunk chunk)
  | EF_vstore chunk ->
      Printf.sprintf "volatile store %s" (name_of_chunk chunk)
  | EF_malloc ->
      "malloc"
  | EF_free ->
      "free"
  | EF_memcpy (sz, al) ->
      Printf.sprintf "memcpy size %s align %s " (Z.to_string sz)
        (Z.to_string al)
  | EF_annot (kind, text, targs) ->
      Printf.sprintf "annot %S" text
  | EF_annot_val (kind, text, targ) ->
      Printf.sprintf "annot_val %S" text
  | EF_inline_asm (text, sg, clob) ->
      Printf.sprintf "inline_asm %S" text
  | EF_debug (kind, text, targs) ->
      Printf.sprintf "debug%d %S" (P.to_int kind) (extern_atom text)
...
let temp_name (id : AST.ident) =
  try "$" ^ Hashtbl.find string_of_atom id
  with Not_found -> Printf.sprintf "$%d" (P.to_int id)
...
let rec print_stmt p s =
  ...
  | Scall (None, e1, el) ->
      fprintf p "@[<hv 2>%a@,(@[<hov 0>%a@]);@]" expr (15, e1) print_expr_list
        (true, el)
  | Scall (Some id, e1, el) ->
      fprintf p "@[<hv 2>%s =@ %a@,(@[<hov 0>%a@]);@]" (temp_name id) expr
        (15, e1) print_expr_list (true, el)
  | Sbuiltin (None, ef, tyargs, el) ->
      fprintf p "@[<hv 2>builtin %s@,(@[<hov 0>%a@]);@]" (name_of_external ef)
        print_expr_list (true, el)
  | Sbuiltin (Some id, ef, tyargs, el) ->
      fprintf p "@[<hv 2>%s =@ builtin %s@,(@[<hov 0>%a@]);@]" (temp_name id)
        (name_of_external ef) print_expr_list (true, el)
  | Ssequence (Sskip, s2) ->
      print_stmt p s2
  | Ssequence (s1, Sskip) ->
      print_stmt p s1
  ...
  | Sifthenelse (e, Sskip, s2) ->
      fprintf p "@[<v 2>if (! %a) {@ %a@;<0 -2>}@]" expr (15, e) print_stmt s2
  | Sifthenelse (e, s1, s2) ->
      fprintf p "@[<v 2>if (%a) {@ %a@;<0 -2>} else {@ %a@;<0 -2>}@]" print_expr
        e print_stmt s1 print_stmt s2
  | Sloop (s1, Sskip) ->
      fprintf p "@[<v 2>while (1) {@ %a@;<0 -2>}@]" print_stmt s1
  | Sloop (s1, s2) ->
      fprintf p "@[<v 2>for (@[<hv 0>;@ 1;@ %a) {@]@ %a@;<0 -2>}@]"
        print_stmt_for s2 print_stmt s1
  | Sbreak ->
      fprintf p "break;"
  | Scontinue ->
      fprintf p "continue;"
...
let print_function ver p id f =
  fprintf p "%s@ "
    (name_cdecl
       (name_function_parameters (name_param ver) (extern_atom id) f.fn_params
          f.fn_callconv )
       f.fn_return ) ;
  fprintf p "@[<v 2>{@ " ;
  Stdlib.List.iter
    ...
    f.fn_vars ;
  Stdlib.List.iter
    (fun (id, ty) -> fprintf p "register %s;@ " (name_cdecl (temp_name id) ty))
    f.fn_temps ;
...
let print_fundecl p id fd =
  match fd with
  | Ctypes.External
      ( (AST.EF_external _ | AST.EF_runtime _ | AST.EF_malloc | AST.EF_free)
      , args
      , res
      , cconv ) ->
      fprintf p "extern %s;@ "
...
let print_globdef var p (id, gd) =
  match gd with
  | AST.Gfun f ->
      print_fundef var p id f
  | AST.Gvar v ->
      print_globvar p id v (* from PrintCsyntax *)

let print_globdecl p (id, gd) =
  match gd with AST.Gfun f -> print_fundecl p id f | AST.Gvar v -> ()
...
let print_program ver p prog =
  ...
  Stdlib.List.iter (print_globdecl p) prog.prog_defs ;
  Stdlib.List.iter (print_globdef ver p) prog.prog_defs ;
  fprintf p "@]@."
