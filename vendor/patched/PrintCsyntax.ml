...
let name_floattype sz = match sz with F32 -> "float" | F64 -> "double"

let name_longtype sg =
  match sg with Signed -> "long long" | Unsigned -> "unsigned long long"

(* Declarator (identifier + type) *)

let attributes a =
  let s1 =
    if a.attr_volatile then
      " volatile"
    else
      ""
  in
  ...
  | Tpointer (t, a) ->
      let id' =
        ...
        | Tfunction _ | Tarray _ ->
            sprintf "(*%s%s)" (attributes_space a) id
        | _ ->
            sprintf "*%s%s" (attributes_space a) id
      in
      name_cdecl id' t
...
let rec precedence = function
  ...
  | Ecast _ ->
      (14, RtoL)
  | Ebinop ((Omul | Odiv | Omod), _, _, _) ->
      (13, LtoR)
  | Ebinop ((Oadd | Osub), _, _, _) ->
      (12, LtoR)
  | Ebinop ((Oshl | Oshr), _, _, _) ->
      (11, LtoR)
  | Ebinop ((Olt | Ogt | Ole | Oge), _, _, _) ->
      (10, LtoR)
  | Ebinop ((Oeq | Cop.One), _, _, _) ->
      (9, LtoR)
  | Ebinop (Oand, _, _, _) ->
      (8, LtoR)
  | Ebinop (Oxor, _, _, _) ->
...
let rec expr p (prec, e) =
  ...
  | Ebuiltin (EF_memcpy (sz, al), _, args, _) ->
      fprintf p "__builtin_memcpy_aligned@[<hov 1>(%ld,@ %ld,@ %a)@]"
        (camlint_of_coqint sz) (camlint_of_coqint al) exprlist (true, args)
  | Ebuiltin (EF_annot (_, txt, _), _, args, _) ->
      fprintf p "__builtin_annot@[<hov 1>(%S%a)@]" txt exprlist (false, args)
  | Ebuiltin (EF_annot_val (_, txt, _), _, args, _) ->
      fprintf p "__builtin_annot_intval@[<hov 1>(%S%a)@]" txt exprlist
        (false, args)
  ...
  | Ebuiltin (EF_debug (kind, txt, _), _, args, _) ->
      fprintf p "__builtin_debug@[<hov 1>(%d,%S%a)@]" (P.to_int kind)
        (extern_atom txt) exprlist (false, args)
  | Ebuiltin (EF_builtin (name, _), _, args, _) ->
      fprintf p "%s@[<hov 1>(%a)@]" name exprlist (true, args)
  | Ebuiltin (_, _, args, _) ->
...
and extended_asm p txt res args clob =
  ...
  | c1 :: cl ->
      ...
      Stdlib.List.iter (fun c -> fprintf p ",@ %S" c) cl ;
      fprintf p "@]"
  end ;
  fprintf p ")@]"
...
let rec print_stmt p s =
  ...
  | Sifthenelse (e, s1, Sskip) ->
      fprintf p "@[<v 2>if (%a) {@ %a@;<0 -2>}@]" print_expr e print_stmt s1
  | Sifthenelse (e, s1, s2) ->
      fprintf p "@[<v 2>if (%a) {@ %a@;<0 -2>} else {@ %a@;<0 -2>}@]" print_expr
        e print_stmt s1 print_stmt s2
  | Swhile (e, s) ->
      fprintf p "@[<v 2>while (%a) {@ %a@;<0 -2>}@]" print_expr e print_stmt s
  | Sdowhile (e, s) ->
      fprintf p "@[<v 2>do {@ %a@;<0 -2>} while(%a);@]" print_stmt s print_expr
        e
  | Sfor (s_init, e, s_iter, s_body) ->
      fprintf p "@[<v 2>for (@[<hv 0>%a;@ %a;@ %a) {@]@ %a@;<0 -2>}@]"
        print_stmt_for s_init print_expr e print_stmt_for s_iter print_stmt
        s_body
  | Sbreak ->
      fprintf p "break;"
  | Scontinue ->
      fprintf p "continue;"
  | Sswitch (e, cases) ->
      fprintf p "@[<v 2>switch (%a) {@ %a@;<0 -2>}@]" print_expr e print_cases
        cases
  | Sreturn None ->
      fprintf p "return;"
  | Sreturn (Some e) ->
...
and print_cases p cases =
  ...
  | LScons (lbl, Sskip, rem) ->
      fprintf p "%a:@ %a" print_case_label lbl print_cases rem
  | LScons (lbl, s, rem) ->
      fprintf p "@[<v 2>%a:@ %a@]@ %a" print_case_label lbl print_stmt s
        print_cases rem
...
and print_case_label p = function
  | None ->
      fprintf p "default"
  | Some lbl ->
      fprintf p "case %s" (Z.to_string lbl)
...
let print_function p id f =
  fprintf p "%s@ "
    (name_cdecl
       (name_function_parameters extern_atom (extern_atom id) f.fn_params
          f.fn_callconv )
       f.fn_return ) ;
  fprintf p "@[<v 2>{@ " ;
  Stdlib.List.iter
...
let print_fundef p id fd =
  match fd with
  | Ctypes.External
      ((EF_external _ | EF_runtime _ | EF_malloc | EF_free), args, res, cconv)
    ->
      fprintf p "extern %s;@ @ "
...
let string_of_init id =
  let b = Buffer.create (Stdlib.List.length id) in
  let add_init = function
    | Init_int8 n ->
        ...
        then
          ...
          Buffer.add_string b (Printf.sprintf "\\%03o" c)
    | _ ->
        assert false
  in
  Stdlib.List.iter add_init id ; Buffer.contents b

let chop_last_nul id =
  match List.rev id with Init_int8 Z.Z0 :: tl -> List.rev tl | _ -> id
...
let print_init p = function
  ...
  | Init_int64 n ->
      fprintf p "%LdLL" (camlint64_of_coqint n)
  | Init_float32 n ->
      fprintf p "%.15F" (camlfloat_of_coqfloat n)
  | Init_float64 n ->
      fprintf p "%.15F" (camlfloat_of_coqfloat n)
  | Init_space n ->
      fprintf p "/* skip %s */@ " (Z.to_string n)
  | Init_addrof (symb, ofs) ->
      let ofs = camlint_of_coqint ofs in
      if ofs = 0l then
        fprintf p "&%s" (extern_atom symb)
      else
        fprintf p "(void *)((char *)&%s + %ld)" (extern_atom symb) ofs
...
let print_globvar p id v =
  ...
  | [] ->
      fprintf p "extern %s;@ @ " (name_cdecl name2 v.gvar_info)
  | [Init_space _] ->
      fprintf p "%s;@ @ " (name_cdecl name2 v.gvar_info)
  | _ ->
      fprintf p "@[<hov 2>%s = " (name_cdecl name2 v.gvar_info) ;
      ...
      | _, il ->
          if false then
            fprintf p "\"%s\"" (string_of_init (chop_last_nul il))
          else
            print_composite_init p il
      end ;
      fprintf p ";@]@ @ "
...
let print_globdecl p (id, gd) =
  match gd with
  | Gfun f ->
      print_fundecl p id f
  | Gvar v ->
      print_globvardecl p id v

let print_globdef p (id, gd) =
  match gd with Gfun f -> print_fundef p id f | Gvar v -> print_globvar p id v
...
let define_composite p (Composite (id, su, m, a)) =
  fprintf p "@[<v 2>%s %s%s {" (struct_or_union su) (extern_atom id)
    (attributes a) ;
  Stdlib.List.iter (print_member p) m ;
  fprintf p "@;<0 -2>};@]@ @ "

let print_program p prog =
  fprintf p "@[<v 0>" ;
  Stdlib.List.iter (declare_composite p) prog.prog_types ;
  Stdlib.List.iter (define_composite p) prog.prog_types ;
  Stdlib.List.iter (print_globdecl p) prog.Ctypes.prog_defs ;
  Stdlib.List.iter (print_globdef p) prog.Ctypes.prog_defs ;
  fprintf p "@]@."
