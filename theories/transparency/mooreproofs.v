...
From Transmogrifier.compiler Require Import moore.
...
Variable states_bounded : Z.of_nat (length moore.(states _)) < Int64.modulus.
...
Variable Hp : compile_program state moore state_eq_dec base = Ok p.
...
Lemma compile_program_defs :
  prog_defs p =
    [ (ids.(id_table),  Gvar (compile_table state moore state_eq_dec));
      (ids.(id_atable), Gvar (compile_atable state moore));
      (ids.(id_delta),  Gfun (compile_delta state moore ids));
      (ids.(id_accept), Gfun (compile_accept state moore ids));
      (ids.(id_q0),     Gvar (compile_q0 state moore state_eq_dec));
      (ids.(id_run),    Gfun (compile_run state moore state_eq_dec ids));
...
Lemma defmap_delta :
  (prog_defmap p) ! (ids.(id_delta)) =
    Some (Gfun (compile_delta state moore ids)).
Proof.
...
Lemma find_delta_def :
  exists b,
    ...
    Genv.find_def ge b = Some (Gfun (compile_delta state moore ids)).
Proof.
...
Lemma find_delta :
  exists b,
    ...
    Genv.find_funct ge (Vptr b Ptrofs.zero) =
      Some (compile_delta state moore ids).
Proof.
...
Definition sidx (q : state) : option Z :=
  index_of state_eq_dec q moore.(states _) 0.
...
Lemma table_row_length : forall q,
  length (table_row state moore state_eq_dec q) = length s.enum.
Proof.
...
Lemma table_entry_correct : forall q sym q_idx s_idx,
  ...
  = Some (Init_int64 (Int64.repr (table_entry state moore state_eq_dec q sym))).
Proof.
  intros q sym q_idx s_idx Hq Hs.
  assert (Hqb : 0 <= q_idx < Z.of_nat (length moore.(states _)))
  ...
  assert (Hrow : nth_error (state_table state moore) (Z.to_nat q_idx) = Some (q_idx, q)).
    { unfold state_table.
      destruct (enumerate_nth_error_pair _ (states state moore) q_idx Hqb)
        as (q' & Hpair & Hnth).
  ...
  assert (Hcol : nth_error (table_row state moore state_eq_dec q) (Z.to_nat s_idx)
                 = Some (Init_int64 (Int64.repr (table_entry state moore state_eq_dec q sym)))).
...
Lemma find_table :
  exists b,
    ...
    Genv.find_def ge b = Some (Gvar (compile_table state moore state_eq_dec)).
Proof.
...
Lemma table_init_all_int64 : forall id,
  In id (table_init state moore state_eq_dec) -> exists x, id = Init_int64 x.
Proof.
...
Variable table_bounded : 8 * (Z.of_nat (length moore.(states _)) * Z.of_nat (length s.enum))
                         < Ptrofs.modulus.

Lemma table_in_mem : forall b k v,
  Genv.find_symbol ge ids.(id_table) = Some b ->
  nth_error (table_init state moore state_eq_dec) (Z.to_nat k) = Some (Init_int64 v) ->
  0 <= k < (nstates state moore) * nsyms ->
...
Proof.
  ...
  assert (Hvi : Genv.find_var_info ge b = Some (compile_table state moore state_eq_dec)).
...
Lemma table_entry_sidx : forall q sym next_idx,
  ...
  table_entry state moore state_eq_dec q sym = next_idx.
...
Lemma atable_init_all_int64 : forall id,
  In id (atable_init state moore) -> exists x, id = Init_int64 x.
Proof.
...
Lemma find_atable :
  exists b,
    ...
    Genv.find_def ge b = Some (Gvar (compile_atable state moore)).
Proof.
  apply Genv.find_def_symbol.
  apply prog_defmap_norepet.
    apply global_idents_norepet.
...
Lemma atable_in_mem : forall b k v,
  Genv.find_symbol ge ids.(id_atable) = Some b ->
  nth_error (atable_init state moore) (Z.to_nat k) = Some (Init_int64 v) ->
  0 <= k < nstates state moore ->
  Mem.loadv Mint64 m0 (Vptr b (Ptrofs.repr (8 * k))) = Some (Vlong v).
Proof.
  intros b k v Hsym Hnth Hk.
  destruct find_atable as (b' & Hsym' & Hdef).
  assert (b' = b) by congruence. subst b'.
  assert (Hvi : Genv.find_var_info ge b = Some (compile_atable state moore)).
    { apply Genv.find_var_info_iff. exact Hdef. }
  destruct (Genv.init_mem_characterization _ _ Hvi Hinit)
    as (_ & _ & Hlsid & _).
  specialize (Hlsid eq_refl).
...
Lemma compile_accept_correct : forall q q_idx,
  ...
  eval_funcall function_entry2 ge m0
    ...
    (Vlong (Int64.repr (accept_entry _ moore q))).
Proof.
  intros q q_idx Hq.
...
Lemma compile_delta_sink : forall q_idx s_idx m,
  ...
  eval_funcall function_entry2 ge m
    ...
    (Vlong (Int64.repr (sink_index state moore))).
Proof.
...
Lemma sidx_run : forall w, exists i, sidx (Moore.run moore w) = Some i.
Proof.
  intros. unfold sidx.
  apply index_of_complete.
  apply Moore.run_in_states.
Qed.
...
Lemma run_snoc : forall w a,
  Moore.run moore (w ++ [a]) = moore.(transition _) (Moore.run moore w) a.
Proof.
  intros. unfold Moore.run.
...
Lemma delta_step_correct : forall w a q_idx a_idx,
  ...
  symidx a = Some a_idx ->
  exists r_idx,
    sidx (Moore.run moore (w ++ [a])) = Some r_idx /\
    eval_funcall function_entry2 ge m0
      (compile_delta state moore ids)
...
Proof.
  ...
  exfalso.
  assert (Hin : In (initial state moore) (states state moore)).
    apply (moore.(states_complete _) []).
...
Lemma run_loop_correct : forall suf pre l_pre l_suf b ofs le q_idx,
  ...
  sidx (Moore.run moore pre) = Some q_idx ->
  le ! (ids.(id_w))   = Some (Vptr b (Ptrofs.repr ofs)) ->
  ...
  exists r_idx le',
    sidx (Moore.run moore (pre ++ suf)) = Some r_idx /\
...
Lemma compile_run_correct : forall w l b ofs,
  ...
  exists r_idx,
    sidx (Moore.run moore w) = Some r_idx /\
    eval_funcall function_entry2 ge m0
      (compile_run state moore state_eq_dec ids)
...
Proof.
  ...
  set (le1 := PTree.set ids.(id_q) (Vlong (Int64.repr (q0_index state moore state_eq_dec)))
  ...
  assert (Hq1 : le1 ! (ids.(id_q))
                = Some (Vlong (Int64.repr (q0_index state moore state_eq_dec)))).
    { subst le1. apply PTree.gss. }
  edestruct (run_loop_correct w [] [] l b ofs le1
    (q0_index state moore state_eq_dec)) as (r_idx & le' & Hr & Hq & Hexec); eauto.
  - constructor.
